...
# Cluster GKE
resource "google_container_cluster" "bis2bis_cluster" {
...
resource "google_container_node_pool" "nodes_primarios" {
  name       = "${google_container_cluster.bis2bis_cluster.name}-node-pool"
  location   = var.region
  cluster    = google_container_cluster.bis2bis_cluster.name
